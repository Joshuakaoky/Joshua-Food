...
import { PantryItemStatus } from '../types/food.js';
...
function normalizeStatus(status?: string): PantryItemStatus {
  if (!status) return 'HAVE';
  const normalized = status.toUpperCase();
  if (normalized === 'HAVE' || normalized === 'LOW' || normalized === 'OUT') return normalized as PantryItemStatus;
  return 'HAVE';
}
